...
public class PostgreSQLPlainTextBackuper extends AbstractPostgreSQLBackuper
{
  ...
  static Trace               logger      = Trace
                                             .getLogger(PostgreSQLPlainTextBackuper.class
                                                 .getName());

  /**
   * The dump format for this (family of) backuper.
   */
  public static final String DUMP_FORMAT = "PostgreSQL Plain Text Dump";
    ...
    try
    {
      ...
      if (useAuthentication)
      {
        ...
        String[] expectFeed = makeExpectDialogueWithAuthentication("pg_dump",
            info, " -f " + fullPath + dumpOptions, login, password, timeout);
      ...
      else
      {
        ...
        String cmd = makeCommand("pg_dump", info, "-f " + fullPath
            + dumpOptions, login);
      ...
      String dumpName, String path, ArrayList tables) throws BackupException
  {
    ...
    try
    {
      if (useAuthentication)
      {
        ...
        expectFeed = makeExpectDialogueWithAuthentication("psql", info,
            "--pset pager -f " + fullPath, login, password, timeout); // 30
        // minutes
        // timeout
      ...
      // No authentication
      {
        ...
        String replayCmd = makeCommand("psql", info, "--pset pager -f "
            + fullPath, login);
  ...
  {
    super.deleteDump(path, dumpName + ".sql");
  }
}
