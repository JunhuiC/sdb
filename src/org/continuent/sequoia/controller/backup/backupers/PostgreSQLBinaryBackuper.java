 ...
 * dumps in a binary format, using the "--format=c" switch on pg_dump.
 * <p>
 * Supported URLs are:
 * <ul>
...
public class PostgreSQLBinaryBackuper extends AbstractPostgreSQLBackuper
{
  ...
  static Trace               logger      = Trace
                                             .getLogger(PostgreSQLBinaryBackuper.class
                                                 .getName());
      ...
      String dumpName, String path, ArrayList tables) throws BackupException
  {
    ...
    try
    {
      ...
      {
        // TODO: Check if this works--it should be disabled?
        if (logger.isDebugEnabled())
        ...
        String[] expectFeed = makeExpectDialogueWithAuthentication("pg_dump",
            info, " --format=c -f " + fullPath + dumpOptions, login, password,
            timeout);

        String[] cmd = makeExpectCommandReadingStdin();

        succeeded = safelyExecNativeCommand(cmd, expectFeed, 0);
      }
      else
      {
        if (pgDumpFlags != null)
        ...
        String cmd = makeCommand("pg_dump", info, " --format=c -f " + fullPath
            + dumpOptions, login);
        succeeded = safelyExecNativeCommand(cmd, null, 0);
      }
      ...
      String dumpName, String path, ArrayList tables) throws BackupException
  {
    ...
    try
    {
      if (useAuthentication)
      {
        ...
        if (! safelyExecNativeCommand(cmd, expectFeed, 0))
        {
          logger.warn("Unable to drop database prior to restore");
        }
        ...
        if (preRestoreScript != null)
        {
          ...
          expectFeed = makeExpectDialogueWithAuthentication(getPsqlCommand(), info,
          ...
          if (! safelyExecNativeCommand(expectFeed, cmd, 0))
          {
            throw new BackupException(
            getPsqlCommand() + " execution did not complete successfully!");
          }
        }
        ...
        expectFeed = makeExpectDialogueWithAuthentication("pg_restore", info,
            "--format=c -d " + info.getDbName() + " " + fullPath, login,
        ...
        if (postRestoreScript != null)
        {
          ...
          if (! safelyExecNativeCommand(expectFeed, cmd, 0))
          {
            throw new BackupException(getPsqlCommand()
      ...
      // No authentication
      {
        ...
        if (! safelyExecNativeCommand(dropCmd, null, 0))
        {
        ...
        String createCmd = makeCommand("createdb", info, encoding != null
            ? "--encoding=" + encoding + " "
            : "", login);
        if (! safelyExecNativeCommand(createCmd, null, 0))
        {
          throw new BackupException(
              "createdb execution did not complete successfully!");
        }
        ...
        if (preRestoreScript != null)
        {
          ...
          String preRestoreCmd = makeCommand(getPsqlCommand(), info, "--pset pager -f "
              + preRestoreScript, login);
          if (! safelyExecNativeCommand(preRestoreCmd, null, 0))
          {
            printErrors();
            throw new BackupException(getPsqlCommand()
                + " execution did not complete successfully!");
        ...
        String replayCmd = makeCommand("pg_restore", info, "--format=c -d "
            + info.getDbName() + " " + fullPath, login);
        if (! safelyExecNativeCommand(replayCmd, null, 0))
        {
          throw new BackupException(
              "pg_restore execution did not complete successfully!");
        }
        ...
        if (postRestoreScript != null)
        {
          ...
          String postRestoreCmd = makeCommand(getPsqlCommand(), info, "--pset pager -f "
              + postRestoreScript, login);
          if (! safelyExecNativeCommand(postRestoreCmd, null, 0))
          {
            throw new BackupException(getPsqlCommand()
                + " execution did not complete successfully!");
          }
