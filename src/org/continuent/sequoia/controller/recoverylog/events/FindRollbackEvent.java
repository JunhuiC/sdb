...
/**
 * This class defines a FindRollbackEvent.
 * <p>
 * This event is used to retrieve a rollback associated to a transaction ID.
 * <br/>Once this event has been executed, the fact that the rollback was found
 ...
 * @version 1.0
 */
public class FindRollbackEvent implements LogEvent
{
   ...
   */
  public FindRollbackEvent(RecoveryLog recoveryLog,
      long requestId)
  {
  ...
  public void execute(LoggerThread loggerThread, RecoveryLogConnectionManager manager)
  {
    ...
    try
    {
      stmt = manager.getConnection().prepareStatement("SELECT log_id, exec_status FROM "
          ...
          + recoveryLog.getLogTableSqlColumnName() + "='rollback'))");
  ...
  {
    return "FindRollbackEvent for request ID " + transactionId;
  }
}
