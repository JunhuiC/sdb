...
/**
 * This class defines a FindCommitEvent.
 * <p>
 * This event is used to retrieve a commit associated to a transaction ID.
 * <br/>Once this event has been executed, the fact that the commit was found or
 * not can be checked through the <code>wasFound()</code> method.
 * 
 * @author <a href="mailto:damian.arregui@continuent.com">Damian Arregui</a>
 * @version 1.0
 */
public class FindCommitEvent implements LogEvent
{
  ...
  /**
   * Creates a new <code>FindCommitEvent</code> object
   * 
   ...
   */
  public FindCommitEvent(RecoveryLog recoveryLog,
      long requestId)
  {
  ...
  public void execute(LoggerThread loggerThread, RecoveryLogConnectionManager manager)
  {
    ...
    try
    {
      stmt = manager.getConnection().prepareStatement("SELECT exec_status FROM "
          + recoveryLog.getLogTableName() + " WHERE ((transaction_id=?) AND ("
          + recoveryLog.getLogTableSqlColumnName() + "='commit'))");
    ...
    catch (SQLException e)
    {
      catchedException = new SQLException(Translate.get(
          "recovery.jdbc.commit.not.found.error", new Object[]{
              new Long(transactionId), e.getMessage()}));
    }
    finally
    {
      try
  ...
  /**
   * Returns the status of the commit statement if found in the recovery log
   * 
   * @return the status the commit statement as found in the recovery log, or an
  ...
  /**
   * Checks if the commit was found.
   * 
   * @return true if the commit has been found, false otherwise
   */
  ...
  {
    return "FindCommitEvent for request ID " + transactionId;
  }
}
