...
/**
 * This class defines a LogCommitEvent to log a Commit or potentially clean the
 * recovery log for that transaction that Commits.
 * 
 * @author <a href="mailto:emmanuel.cecchet@emicnetworks.com">Emmanuel Cecchet</a>
 * @version 1.0
 */
public class LogCommitEvent extends LogRequestEvent
{
  /**
   * Creates a new <code>LogCommitEvent</code> object
   * 
   * @param entry a log entry that describes a Commit (the SQL query is ignored)
   */
  public LogCommitEvent(LogEntry entry)
  {
    super(entry);
  }
  ...
  public void execute(LoggerThread loggerThread, RecoveryLogConnectionManager manager)
  {
    ...
    // First let's try to remove the empty transaction (if empty) from the log
    // if no-one is currently processing the log
    try
    {
      if (!loggerThread.getRecoveryLog().isRecovering())
      {
        if (loggerThread.removeEmptyTransaction(manager, logEntry.getTid()))
          return;
      }
    }
    catch (SQLException e)
    {
      ...
      logger.error(Translate.get("recovery.jdbc.loggerthread.log.failed",
          new String[]{"Commit", String.valueOf(logEntry.getTid())}), e);
      // Fallback to default logging
    }

    // Ok, someone has a lock on the log, just log the Commit.
