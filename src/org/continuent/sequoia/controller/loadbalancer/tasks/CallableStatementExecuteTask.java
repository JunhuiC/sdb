 ...
 * @version 1.0
 */
public class CallableStatementExecuteTask extends AbstractTask
{
   ...
   */
  public CallableStatementExecuteTask(int nbToComplete, int totalNb,
      ...
      throws SQLException
  {
    ...
    try
    {
      result = AbstractLoadBalancer.executeCallableStatementExecuteOnBackend(
          proc, backend, backendThread, c, metadataCache);
      ...
      throws SQLException
  {
    ...
    {
      result = AbstractLoadBalancer.executeCallableStatementExecuteOnBackend(
          proc, backend, backendThread, cm
              .retrieveConnectionForTransaction(tid), metadataCache);
  ...
  {
    if ((other == null) || !(other instanceof CallableStatementExecuteTask))
      return false;

    CallableStatementExecuteTask cset = (CallableStatementExecuteTask) other;
    if (proc == null)
      return cset.getRequest() == null;
    return proc.equals(cset.getRequest());
  }
  ...
  {
    if (proc.isAutoCommit())
      return "Autocommit CallableStatementExecuteTask "
          + proc.getTransactionId() + " (" + proc.getUniqueKey() + ")";
    else
      return "CallableStatementExecuteTask for transaction "
          + proc.getTransactionId() + " (" + proc.getUniqueKey() + ")";
  }
