...
public abstract class AbstractTask
{
  ...
  private List<BackendWorkerThread>      notifications  = null;
  ...
  private Map<DatabaseBackend, List<TransactionLogicalLock>>       locksMap       = new HashMap<DatabaseBackend, List<TransactionLogicalLock>>();
      ...
      boolean isPersistentConnection, long persistentConnectionId)
  {
    ...
    failed = 0;
    executionStarted = 0;
    notifications = new ArrayList<BackendWorkerThread>(nbToComplete);
   ...
   */
  public synchronized void setLocks(DatabaseBackend backend, List<TransactionLogicalLock> locks)
  {
    synchronized (locksMap)
    {
      /*
  ...
  {
    List<TransactionLogicalLock> locks = (List<TransactionLogicalLock>) locksMap.get(backend);
    if (locks == null)
    {
      locks = new ArrayList<TransactionLogicalLock>();
      locksMap.put(backend, locks);
    }
