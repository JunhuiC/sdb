...
public class RAIDb1DistributedRequestManager extends DistributedRequestManager
{
      ...
      long messageTimeout) throws SQLException
  {
    ...
    try
    {
      ...
      { // Notify all controllers of completion
         ...
         * completion was 'success'. We determine success from the local result
         * or the successful remote controllers even though they may have failed
         * afterwards (in which case requestResult may be null).
         */
      ...
      long messageTimeout) throws SQLException
  {
    ...
    try
    {
       ...
       */
      boolean success = (requestResult != null || successfulControllers != null);
      notifyRequestCompletion(request, success, true, failedOnAllBackends);
      ...
      long messageTimeout) throws SQLException
  {
    ...
    try
    {
      ...
      int requestUpdateCount = -1;
      if (requestResult != null)
        requestUpdateCount = requestResult.getUpdateCount();
      ...
      notifyRequestCompletion(request, requestResult != null, true,
          failedOnAllBackends, requestUpdateCount);

      // Notify controllers without active backends (if any)
      notifyRequestCompletion(request, requestResult != null, false,
          controllersWithoutBackends, requestUpdateCount);
      ...
      long messageTimeout) throws SQLException
  {
    ...
    try
    {
      ...
      /*
       * Notify all controllers where all backend failed (if any) that
       * completion was 'success'. We determine success from the local result or
       * the successful remote controllers even though they may have failed
       * afterwards (in which case requestResult may be null).
       */
      boolean success = (requestResult != null || successfulControllers != null);
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        if (r instanceof StoredProcedureCallResult)
        {
          ...
          if (result.getResult() instanceof ExecuteUpdateResult
              && ((StoredProcedureCallResult) r).getResult() instanceof ExecuteUpdateResult)
          {
            ExecuteUpdateResult requestResult = ((ExecuteUpdateResult) result
                .getResult());
            ExecuteUpdateResult executeUpdateResult = ((ExecuteUpdateResult) (((StoredProcedureCallResult) r)
                .getResult()));
            if (requestResult.getUpdateCount() != executeUpdateResult
                .getUpdateCount())
            {
              if (inconsistentControllers == null)
                inconsistentControllers = new ArrayList();
              ...
              logger.error("Controller " + member
                  + " returned an inconsistent results ("
                  + executeUpdateResult.getUpdateCount() + " when expecting "
                  + requestResult.getUpdateCount()
                  + ") for stored procedure " + proc.getId());
            }
          }
        }
        else if (DistributedRequestManager.SUCCESSFUL_COMPLETION.equals(r))
      ...
      if (inconsistentControllers != null)
      { // Notify all inconsistent controllers to disable their backends
        notifyControllerInconsistency(proc, inconsistentControllers);
      }
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for abort of transaction " + transactionId);
        }
      }

      // Notify all controllers that have thrown an exception
      notifyRequestCompletion(abortRequest, success, false,
          controllersWithException);
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          ...
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for commit of transaction " + transactionId);
        }
      }
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          ...
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for rollback of transaction " + transactionId);
        }
      }
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          ...
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for rollback to savepoint " + savepointName
                + " of transaction " + transactionId);
        }
      }
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          ...
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for set savepoint " + name + " of transaction "
                + transactionId);
        }
      }
    ...
    try
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        else
        {
          ...
          if (logger.isWarnEnabled())
            logger.warn("Unexpected answer from controller " + member + " ("
                + r + ") for release savepoint " + name + " of transaction "
                + transactionId);
        }
      }
