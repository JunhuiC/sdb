...
import org.continuent.sequoia.controller.cache.result.entries.ResultCacheEntryNoCache;
import org.continuent.sequoia.controller.cache.result.entries.ResultCacheEntryRelaxed;
import org.continuent.sequoia.controller.cache.result.schema.CacheDatabaseSchema;
import org.continuent.sequoia.controller.cache.result.schema.CacheDatabaseTable;
import org.continuent.sequoia.controller.cache.result.threads.EagerCacheThread;
import org.continuent.sequoia.controller.cache.result.threads.RelaxedCacheThread;
...
public abstract class ResultCache extends AbstractResultCache
{
  ...
  /**
   * @see org.continuent.sequoia.controller.cache.result.AbstractResultCache#getDefaultRule()
   */
  ...
  /**
   * @see org.continuent.sequoia.controller.cache.result.AbstractResultCache#setDefaultRule(ResultCacheRule)
   */
  ...
  private CacheBehavior getCacheBehavior(SelectRequest request)
  {
    ...
    if (logger.isDebugEnabled())
      logger.debug(Translate.get("resultcache.behavior.for.request",
          new String[]{request.getUniqueKey(), behavior.getType()}));
    return behavior;
  }
  ...
  {
    return request.getLogin() + "," + request.getUniqueKey();
  }
      ...
      throws CacheException
  {
    ...
    if (request.getCacheAbility() == RequestType.UNCACHEABLE)
      throw new CacheException(Translate.get("resultcache.uncacheable.request",
  ...
  {
    String sqlQuery = request.getUniqueKey();
  ...
  public void writeNotify(AbstractWriteRequest request) throws CacheException
  {
    ...
    if (logger.isDebugEnabled())
      logger.debug("Notifying write " + request.getUniqueKey());
  ...
  private void removeOldest()
  {
     ...
     */
    queries.remove(oldce.getRequest().getUniqueKey());
  ...
  /**
   * @see org.continuent.sequoia.controller.cache.result.AbstractResultCache#getCacheData
   */
  ...
  /**
   * @see org.continuent.sequoia.controller.cache.result.AbstractResultCache#getCacheStatsData()
   */
