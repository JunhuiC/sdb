 ...
 * van Zuylen, Marc Herbert, Edward Archibald, Stephane Giron.
 */
      ...
      java.lang.Cloneable
{
  ...
  /** the warning chain */
  protected BackendDriverException          warnings             = null;
  /** warnings associated to the statement that created us */
  protected SQLWarning                      statementWarnings    = null;
  ...
  /**
   * Hold a reference to the returned stream so we can close it as soon as the
   * application is no more allowed to use it (per the JDBC spec).
   */
  ...
  /**
   * See performance issue explained in
   * {@link org.continuent.sequoia.controller.virtualdatabase.VirtualDatabaseWorkerThread#fetchNextResultSetRows()}
   * 
  ...
  public String getString(int columnIndex) throws SQLException
  {
    ...
    {
      Object tmpData = (((Object[]) data.get(currentRow))[columnIndex - 1]);
      if (tmpData instanceof byte[])
      {
        String encoding = this.fields[columnIndex - 1].getEncoding();
        if (encoding != null)
          try
          {
            return new String((byte[]) tmpData, encoding);
          }
          catch (UnsupportedEncodingException e)
          {
            throw (SQLException) new SQLException(
                "UnsupportedEncodingException (to " + encoding + ") : "
                    + e.getMessage()).initCause(e);
          }
        else
          return new String((byte[]) tmpData);
      }
  ...
  public boolean getBoolean(int columnIndex) throws SQLException
  {
    ...
    if ((stringVal != null) && (stringVal.length() > 0))
    {
      ...
      throw new SQLException("column value " + stringVal + " of type: "
          + object.getClass() + " could not be converted to boolean");
    }
    else
    {
      return false;
  ...
  public short getShort(int columnIndex) throws SQLException
  {
    ...
    {
      // Try to convert a boolean
      String string = obj.toString().toLowerCase().trim();
  ...
  public byte[] getBytes(int columnIndex) throws SQLException
  {
    ...
    byte[] maybeEncodedBytes;
    if (o instanceof byte[])
    {
      maybeEncodedBytes = (byte[]) o;
    }
    else if (o instanceof ByteArrayBlob)
    {
      ByteArrayBlob b = (ByteArrayBlob) o;
      // TODO: use getBinaryStream() to avoid the copy
      maybeEncodedBytes = b.getBytes(1, (int) b.length());
    }
    else
    {
      throw new NotImplementedException("in getBytes(" + columnIndex
          + "), don't know how to convert type " + o.getClass().getName()
          + " into a byte array ");
    }

    return maybeEncodedBytes; // maybe not!
  }
  ...
  public java.sql.Date getDate(int columnIndex) throws SQLException
  {
    ...
    // if we already have a date object, avoid conversions
    if (o instanceof java.sql.Date)
      return (java.sql.Date) o;

    // otherwise, try to guess the Date by converting the object to a string
    ...
    {// MySQL year(2)
      int year = Integer.parseInt(dateString);
      // got to guess if it's 19XX or 20XX
      if (year < 1970)
        year += 100;
      Calendar cal = new GregorianCalendar();
      cal.clear();
    ...
    { // don't know what that can be, let Date type try to parse it
      return java.sql.Date.valueOf(dateString);
    }
  }
  ...
  public InputStream getAsciiStream(int columnIndex) throws SQLException
  {
    ...
    try
    { // TODO: actually implement a "streaming" encoding instead of encoding
      // everything at once. See SEQUOIA-400
      currentStream = new ByteArrayInputStream(Charset.forName("US-ASCII")
          .newEncoder().encode(CharBuffer.wrap(getString(columnIndex))).array());
    }
    catch (CharacterCodingException e)
    {
      throw (SQLException) new SQLException(
          "Caught CharacterCodingException (data is probably not ASCII): "
              + e.getMessage()).initCause(e);
    }
  ...
  public InputStream getUnicodeStream(int columnIndex) throws SQLException
  {
    ...
    currentStream = new ByteArrayInputStream(getString(columnIndex).getBytes());
    return currentStream;
  }
  ...
  public InputStream getBinaryStream(int columnIndex) throws SQLException
  {
    ...
    if (b != null)
    {
      currentStream = new ByteArrayInputStream(b);
      return currentStream;
    }
    else
  ...
  {
    checkIfClosed();
    return connection.convertToSQLWarnings(warnings);
  }
  ...
  void setStatementWarnings(SQLWarning sqlw)
  {
    statementWarnings = sqlw;
  }
...
//  }
  ...
  public java.sql.Clob getClob(int columnIndex) throws SQLException
  {
    ...
    if (o instanceof String)
      ...
      // FIXME: Huh? What it is the purpose of this?
      return new StringClob(new String(o.toString()));
  }
...
//  }
  ...
  public void close() throws SQLException
  {
    ...
    closeCurrentStreamIfExists();

    if (hasMoreData)
      this.connection.closeRemoteResultSet(cursorName);
    isClosed = true;
    // garbage collect some data
    this.data = null;
    this.fields = null;
  ...
  {
    if (columnName == null)
      throw new java.sql.SQLException("Invalid null column name");
   ...
   * an input stream. MUST mirror the following serialization method
   * {@link org.continuent.sequoia.controller.backend.result.ControllerResultSet#sendToStream(org.continuent.sequoia.common.stream.DriverBufferedOutputStream)}
   * 
  ...
  public DriverResultSet(Connection conn) throws IOException, ProtocolException
  {
    ...
    DriverBufferedInputStream input = this.connection.socketInput;
    // Get the warning chain
    if (input.readBoolean())
    {
      warnings = new BackendDriverException(input);
    }
    else
      warnings = null;
    ...
    if (this.nbOfRows > 0)
    {
      ...
      for (int col = 0; col < this.nbOfColumns; col++)
      {
        ...
        try
        {
          if (!tag.equals(TypeTag.JAVA_NULL))
    ...
    if (this.hasMoreData)
      this.cursorName = input.readLongUTF();
 ...
 /**
   * Creates a new empty <code>DriverResultSet</code> object
   * 
   * @param empty MUST be true
   */
  DriverResultSet(Connection conn, boolean empty)
  {
    if (!empty)
      throw new IllegalArgumentException(
          "empty flag must be true in DriverResultSet constructor");
  ...
  /**
   * Tells the controller we don't need this ResultSet anymore (ie. closes the
   * remote ResultSet)
   * 
   * @see #close()
   */
  ...
  /**
   * Creates a new <code>DriverResultSet</code> object with given data
  ...
  public DriverResultSet(Connection conn, ArrayList<?> fromData, Field fromField)
  {
    ...
    if (fromData != null && fromData.size() > 0)
    {
      ...
      for (int i = 0; i < nbOfRows; i++)
      {
        ...
        row[0] = fromData.get(i);
        this.data.add(row);
      }
      this.fields = new Field[1];
      this.fields[0] = fromField;
    }
  }

  /**
   * De-serialize only data rows, not any metadata. Useful for streaming. This
   * method MUST mirror the following serialization method:
   * {@link org.continuent.sequoia.controller.backend.result.ControllerResultSet#sendRowsToStream(org.continuent.sequoia.common.stream.DriverBufferedOutputStream)}
   * 
   * @throws IOException stream error
   * @throws ProtocolException protocol corruption
   */
  private void receiveRows() throws IOException, ProtocolException
  {

    DriverBufferedInputStream input = connection.socketInput;
    this.nbOfRows = input.readInt();
    ...
    for (int r = 0; r < this.nbOfRows; r++)
    {
       ...
       * <p>
       * (1) we went through {@link #DriverResultSet(Connection)} at least once
       * before
       * <p>
       * (2) and there was a non-zero ResultSet transfered, else we would not
       * come here again.
       */
      for (int col = 0; col < this.nbOfColumns; col++)
        if (nulls[col])
        ...
        else
        {
          if(serializers[col] == null)
            // We receive data for which we did not get the serializer yet so far
            serializers[col] = SQLDataSerialization.getSerializer(new TypeTag(input));
  ...
  /**
   * Builds a hash between column names and their indices for fast retrieval. An
  ...
  private void buildIndexMapping(Hashtable<String, Integer> table, boolean sensitive)
  {
    ...
    {
      // pgjdbc and MySQL connector/J < 5 make no difference between name and
      // label. See SEQUOIA-746 and SEQUOIA-218
      putFieldNameInIndexMapping(fields[i].getFieldLabel(), i, sensitive, table);
      putFieldNameInIndexMapping(fields[i].getFieldName(), i, sensitive, table);

      /*
       * Add the fully qualified name in any case. It is fully qualified and
       * guaranteed, therefore, not to be a duplicate unless it really is the
       * same column in the same table ...
       */
      putFieldNameInIndexMapping(fields[i].getFullName(), i, sensitive, table);
    }
  }
      ...
      boolean sensitive, Hashtable<String, Integer> table)
  {
    ...
    String key = sensitive ? name : name.toLowerCase();
  ...
  private void extractPrimaryKey() throws SQLException
  {
    ...
    if (fields[0].getTableName() == null
        || fields[0].getTableName().length() == 0)
      throw new SQLException("Could not extractPrimaryKeys: empty tablename");

    ResultSet res = this.connection.getPrimaryKeys(null, null, fields[0]
        .getTableName());
  ...
  /**
   * Check if the ResultSet can be scrolled.
   * 
   * @throws SQLException if the ResultSet is closed or if its type is
   *           TYPE_FORWARD_ONLY.
   */
  private void checkIfScrollable() throws SQLException
  {
    checkIfClosed();
    if (resultSetType == ResultSet.TYPE_FORWARD_ONLY)
    {
  ...
  /**
   * Close a previously created input stream if needed.
   */
  private void closeCurrentStreamIfExists()
  {
    if (currentStream != null)
    {
      try
      {
        // since we use only ByteArrayInputStreams for now, this is a no-op.
        currentStream.close();
      }
