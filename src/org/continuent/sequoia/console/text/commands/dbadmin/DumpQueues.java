...
/**
 * This class defines the command used to dump the backend task queues.
 */
public class DumpQueues extends AbstractAdminCommand
{

  /**
   * Creates a new <code>DumpQueues</code> object
   * 
   * @param module the commands is attached to
   */
  public DumpQueues(VirtualDatabaseAdmin module)
  {
    super(module);
  }
  ...
  public void parse(String commandText) throws Exception
  {
    ...
    String backend = commandText.trim();
    BackendTaskQueuesControlMBean taskQueues = null;
    try {
      taskQueues = jmxClient.getBackendTaskQueues(
          dbName, backend, user, password);
    } catch (Exception e)
    {
      console.printInfo(ConsoleTranslate.get("DumpQueues.noQueues")); //$NON-NLS-1$
    }
    console.print(taskQueues.dump());
  }
  ...
  {
    return ConsoleTranslate.get("DumpQueues.params"); //$NON-NLS-1$
  }
  ...
  {
    return ConsoleTranslate.get("DumpQueues.description"); //$NON-NLS-1$
  }
}
