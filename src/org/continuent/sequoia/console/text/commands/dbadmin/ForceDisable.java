...
public class ForceDisable extends AbstractAdminCommand
{
  ...
  public void parse(String commandText) throws Exception
  {
    ...
    for (int i = 0; i < backendNames.length; i++)
    {
      ...
      console.printInfo(ConsoleTranslate.get(
          "admin.command.force.disable.backend", backendName)); //$NON-NLS-1$
      }
  }
  ...
  {
    return ConsoleTranslate.get("admin.command.force.disable.description"); //$NON-NLS-1$
  }
  ...
  {
    return "force disable"; //$NON-NLS-1$
  }
  ...
  {
    return ConsoleTranslate.get("admin.command.force.disable.params"); //$NON-NLS-1$
  }
