...
public class SetSavePoint extends ConsoleCommand
{
  ...
  public void parse(String commandText) throws IOException, ConsoleException
  {
    ...
    try
    {
      ...
      console.printInfo(ConsoleTranslate.get("sql.command.savepoint.saved", name)); //$NON-NLS-1$
    }
    catch (Exception e)
    {
  ...
  {
    return ConsoleTranslate.get("sql.command.savepoint.params"); //$NON-NLS-1$
     }
  ...
  {
    return ConsoleTranslate.get("sql.command.savepoint.description"); //$NON-NLS-1$
  }
}