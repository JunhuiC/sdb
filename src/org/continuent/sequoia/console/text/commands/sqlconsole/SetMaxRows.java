...
public class SetMaxRows extends ConsoleCommand
{
  ...
  public void parse(String commandText) throws IOException, ConsoleException
  {
    try
    {
      ...
      console.printInfo(ConsoleTranslate.get("sql.command.maxrows.value", //$NON-NLS-1$
          maxrows));
    }
  ...
  {
    return ConsoleTranslate.get("sql.command.maxrows.params"); //$NON-NLS-1$
  }
  ...
  {
    return ConsoleTranslate.get("sql.command.maxrows.description"); //$NON-NLS-1$
  }
}