...
public class SetIsolation extends ConsoleCommand
{
  ...
  public void parse(String commandText) throws IOException, ConsoleException
  {
    try
    {
      ...
      console.printInfo(ConsoleTranslate.get("sql.command.isolation.value", //$NON-NLS-1$
          isolation));
    }
    ...
    {
      throw new ConsoleException(ConsoleTranslate.get("sql.command.isolation.failed"), e); //$NON-NLS-1$
    }
  }
  ...
  {
    return ConsoleTranslate.get("sql.command.isolation.params"); //$NON-NLS-1$
  }
  ...
  {
    return ConsoleTranslate.get("sql.command.isolation.description"); //$NON-NLS-1$
  }
}